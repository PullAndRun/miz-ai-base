...
- `[miz.vtb]`：B 站用户、卡片、直播和动态接口，以及群订阅列表。直播状态按 `cron` 批量检查；动态通过 `dynamicPollMinutes` 分片轮转，`dynamicConcurrency` 控制并发；粉丝卡片按 `cardCacheMinutes` 缓存。每个 `[[miz.vtb.subscriptions]]` 可用 `atAllStreamers = ["主播名"]` 单独指定开播时 @全体成员；仅当机器人是群主或管理员，且 QQ 账号在该群仍有可用的 @全体 次数时生效，否则发送普通开播通知。动态和下播通知不会 @。
...
- VTB：默认每 3 分钟批量检查直播状态，动态分片轮转并在约 15 分钟内覆盖全部订阅主播，粉丝数默认缓存 30 分钟。
- VTB 上游请求会自动合并相同的并发查询并限制请求间隔；遇到 429/412 或连续上游故障时会暂时停止请求，冷却后再恢复。
