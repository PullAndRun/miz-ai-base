  ...
  async handle({ command, config, logger, message, reply, replyWithoutRetry }) {
    ...
    try {
      ...
      if (duration === undefined) {
        await reply("没读到视频时长。链接可能已失效、需要登录，或内容没有公开。换一个能直接打开的链接试试吧。");
        return;
      }

      if (!isVideoDurationAllowed(duration)) {
        await reply("这个视频超过 10 分钟，暂时发不了。可以换个短版，或者先裁剪一下。");
        return;
      }
      ...
      await reply("视频刚才没处理成功，可以稍后再试。如果内容需要登录，请让管理员检查对应站点的登录配置。");
    }
  },
};
