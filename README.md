...
- 参考价达到 `sellPrice` 就标成「达到售卖线」，说明这个价格值得上线挂单。推送里只列这些商品及其前几条挂单；在售不足 `batchSampleSize` 条（默认 5 条）的稀薄行情会单独列一组并标出在售条数；其余只统计数量，一个都没列出来时只回一句提示，不发合并转发。不配置 `sellPrice` 时没有达标一说，按行情全部列出。
