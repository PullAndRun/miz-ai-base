...
| 娱乐功能 | 占卜、B 站礼物特效与礼物抽奖、米哈游笑话图、文字和图片复读 |
...
| `miz 礼物 <礼物名>` | 用一条合并转发消息发礼物：礼物名、礼物 ID、价格（付费礼物换算成电池与人民币）、特效 ID 等台账数据，加上礼物的展示效果；同名礼物取礼物 ID 最大的一版，有全屏特效的送特效视频，没有的送礼物动图；名字后面加 `动图` 可以只看动图，`miz vtb gift <礼物名>` 等价。 |
| `miz 抽奖` | 迷子的小游戏：从素材库里随机抽一款 B 站礼物，用合并转发附上抽奖揭晓和它的展示效果；礼物按价值分五档稀有度（🌱 普通 / ⭐ 稀有 / ✨ 史诗 / 💎 传说 / 👑 神话），越贵越难抽到。抽到的礼物按价值折算成**迷币**（免费礼物保底 1 迷币），奖品行会显示本次获得和该群累计；每个群每天只有一次机会（谁先抽到算谁的，跨群互不影响，私聊按用户各算一次），迷币按群按人记录在 PostgreSQL 的 `gift_lottery_coin_balances` 表里。 |
| `miz 抽奖 榜单` | 看本群迷币榜前 10 名（用 `@` 点名），不在榜上时会在末尾附上自己的名次；只看榜不消耗每天的抽奖名额。 |
...
多数英文命令同时提供中文别名，具体以 `miz help` 的输出为准。`miz 礼物` 读取项目根目录的 `resource/bili-gift` 素材库（`素材索引.json` 与各素材文件）；素材库体积较大，未提交到仓库，需要自行放入该目录，缺失时命令会提示管理员检查。
