...
| 娱乐功能 | 占卜、B 站礼物抽奖与礼物特效查询、米哈游笑话图、文字和图片复读 |
...
| `[miz.recall]` | 可以撤回迷子群消息的用户白名单；群主和管理员无需加入白名单。 |
...
在 `config/vtb.toml` 的订阅块中设置 `contributionStreamers = ["主播昵称"]`，即可为该群开启实时事件感谢；该主播必须同时存在于 `streamers`。舰长、提督、总督加入大航海和续费都会推送，普通礼物累计价值达到 `miz.vtb.contributionMinAmount` 元才推送，直播间红包开始时也会提醒群员进入直播间抢红包。执行 `miz vtb unsubscribe contribution <主播昵称>` 后，该群的打赏、舰长、续费和红包等所有实时事件都会关闭；事件仍会保留用于下播汇总。
...
- 参考价达到 `sellPrice` 就标成「达到售卖线」，说明这个价格值得上线挂单；在售不足 `batchSampleSize` 条（默认 5 条）的稀薄行情也会列出来并标出在售条数。推送只有一行一个商品：`✅ 风之晶簇 148｜前5 146/148/148/150/154`、`🔍 雷之晶簇 15｜在售2 14/15`。其余商品不列出，一个都没有时只回一句提示，不发合并转发。不配置 `sellPrice` 时没有达标一说，按行情全部列出。
- 打开 `alertEnabled` 后，会按 `[miz.ff14].priceAlertCron` 的节奏（默认每小时）自动推送售卖提醒：参考价达到售卖线、或在售不足 `batchSampleSize` 条的商品才会推，同一个价位只提醒一次，价位变了才再提醒；`alertAtUserIds` 指定提醒时要 `@` 的成员，留空就只发群消息。
...
| `miz video <URL>` | 下载并发送视频；普通成员仅可使用 B 站链接，白名单成员可使用其他 yt-dlp 支持的站点。 |
...
| `miz vtb list/subscribe/unsubscribe ...` | 查看或维护本群订阅；支持 `live`、`dynamic`、`contribution`，`list` 会列出实时打赏感谢状态；需要群管理员或 VTB 管理员白名单权限。 |
...
| `miz vtb gift <礼物名>` | 用一条合并转发消息发指定礼物的特效：礼物名、礼物 ID、价格（电池与人民币）、特效 ID 等台账数据，加上礼物的展示效果；同名礼物取礼物 ID 最大的一版，有全屏特效的送特效视频，没有的送礼物动图，名字后面加 `动图` 可以只看动图。 |
...
| `miz ff14 batch [群号]` | 批量查价：查询 `config/ff14.toml` 中该群的商品清单，每种商品取最低 5 条挂单的中位价作参考；只列出参考价达到售卖线 `sellPrice` 的商品，以及在售不足 5 条的稀薄行情，一个都没有时只回一句提示。带群号时可以从别的群或私聊触发，需要群管理或 FF14 管理白名单权限。 |
...
| `miz recall [数量]` / `miz 撤回 [数量]` | 从群消息历史中查找并撤回迷子账号最近发送的消息（包括同一账号从手机等其他客户端发出的消息），数量默认为 1、单次最多 20；仅 `[miz.recall].whitelistUserIds` 中的 QQ 号可用。成功后静默，超时或部分失败时会提示结果。 |
| `miz joke` | 随机发送 10 张不重复的米哈游笑话图。 |

多数英文命令同时提供中文别名，具体以 `miz help` 的输出为准。`miz 抽奖` 和 `miz vtb gift` 读取项目根目录的 `resource/bili-gift` 素材库（`素材索引.json` 与各素材文件）；素材库体积较大，未提交到仓库，需要自行放入该目录，缺失时命令会提示管理员检查。特效视频本身坏了（结构损坏会让 QQ 提示播放失败）时会自动改用该礼物的动图，并在日志里记一条 warn，方便管理员替换素材。
...
| FF14 售卖提醒 | 与低价提醒共用同一个 cron；`batchQueries` 中打开 `alertEnabled` 的清单会在参考价达到 `sellPrice`、或在售不足 `batchSampleSize` 条时推送，价位记录到 PostgreSQL，同一个价位只提醒一次，参考价变化后才再提醒。 |
